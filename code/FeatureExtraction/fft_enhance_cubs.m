...
%--------------------------------------------------------------------------
function [enhimg, imgCoherence, imgOrientation,fimg,bwimg,eimg] =  fft_enhance_cubs(img, BLKSZ)
    global NFFT;
    ...
    %allocate outputs
    %-------------------------
    imgOrientation        =   zeros(nBlkHt,nBlkWt);
    ...
    for i = 0:nBlkHt-1
        ...
        for j = 0:nBlkWt-1
            ...
            imgOrientation(i+1,j+1)   =   compute_mean_angle(dEnergy,th);         %ridge angle
    ...
    %-------------------------
    for i = 1:3
        imgOrientation = smoothen_orientation_image(imgOrientation);            %smoothen orientation image
    end;
    fimg    =   smoothen_frequency_image(fimg,RMIN,RMAX,5); %diffuse frequency image
    imgCoherence    =   compImgCoh(imgOrientation);                    %coherence image for bandwidth
    bwimg   =   get_angular_bw_image(imgCoherence);                 %QUANTIZED bandwidth image
    %-------------------------
    %FFT reconstruction
    %-------------------------
    for i = 0:nBlkHt-1
        for j = 0:nBlkWt-1
            ...
            %reconstruction
            %--------------------------
            af      =   get_angular_filter(imgOrientation(i+1,j+1),bwimg(i+1,j+1),angf_pi_4,angf_pi_2);
            blkfft  =   blkfft.*(af); 
