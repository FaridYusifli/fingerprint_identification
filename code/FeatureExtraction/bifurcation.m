function [res, progress, sx, sy, angle] = bifurcation(img, x, y, o, core_x, core_y)
% -testing bifurcation
   iax = 0; iay = 0; ibx = 0; iby = 0; icx = 0; icy = 0;
   progress = 1;
   path_length = 4; 

% if the single ridge in a bifurcation is the going the same direction as the orientation (res=3)
% or in in the opposite (res=2)
   ...
   hist=[]; % -histogram.but we can use also HISTOGRAM
   hist(1,:)=[pay, pax];    hist(2,:)=[pby, pbx];
   hist(3,:)=[pcy, pcx];    hist(4,:)=[y, x];
   stop=0;
   while ( progress < path_length && ~stop)
      progress = progress + 1;
      da = 0; db = 0; dc = 0;
      if pao ~= 0 
          i = 1;
